...
const moduleTag = "@mjljm/effect-date/Token/";
...
const threeDigits = new RegExp(
	JsRegExp.digit + JsRegExp.digit + JsRegExp.digit,
);
const fourDigits = new RegExp(
	JsRegExp.digit + JsRegExp.digit + JsRegExp.digit + JsRegExp.digit,
);
...
const disallowedTokenValue =
	...
	(
		tokenValue: string,
	): Either.Either<number, MBadArgumentError.DisallowedValue<string>> =>
		pipe(
			tokenValue,
	...
	> =>
		Either.gen(function* () {
			...
			return pipe(
				...
				Array.map((n) =>
					Tuple.make(localeIntl.format(new Date(2023, 10, n)), n),
				),
				HashMap.fromIterable,
			);
		}),
...
const array = Array.make(
	...
	Descriptor({
		...
		tokenToMergedToken: MFunction.memoize(Infinity)(
			(locale: string | undefined) =>
				pipe(
					monthsMapForLocale({ format: "short", locale }),
					Either.map(disallowedTokenValue),
				),
		),
		mergedToken: "month",
	}),
	...
	Descriptor({
		...
		tokenToMergedToken: MFunction.memoize(Infinity)(
			(locale: string | undefined) =>
				pipe(
					monthsMapForLocale({ format: "long", locale }),
					Either.map(disallowedTokenValue),
				),
		),
		mergedToken: "month",
	...
	Descriptor({
		...
		tokenToMergedToken: MFunction.memoize(Infinity)(
			(locale: string | undefined) =>
				pipe(
					weekDaysMapForLocale({ format: "short", locale }),
					Either.map(disallowedTokenValue),
				),
		),
		mergedToken: "weekDay",
	...
	Descriptor({
		...
		tokenToMergedToken: MFunction.memoize(Infinity)(
			(locale: string | undefined) =>
				pipe(
					weekDaysMapForLocale({ format: "long", locale }),
					Either.map(disallowedTokenValue),
				),
		),
		mergedToken: "weekDay",
...
export const [tokens, [tokenPatterns, [mergedTokens, tokenToMergedTokens]]] =
	pipe(
		array,
		Array.map(
			({ name, label, parsePattern, tokenToMergedToken, mergedToken }) => ({
				parsePattern,
				mergedToken: {
					key: mergedToken,
					descriptor: pipe(
				...
				tokenToMergedToken: flow(
					...
					Either.map(
						compose(
							Either.mapLeft(
								MBadArgumentError.mapId(() => `${token}(${label})`),
							),
						),
					),
				),
