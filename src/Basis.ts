import * as Errors from "#mjljm/effect-date/Errors";
import * as MergedBasis from "#mjljm/effect-date/MergedBasis";
import * as MergedToken from "#mjljm/effect-date/MergedToken";
import { MArray, MFunction } from "#mjljm/effect-lib";
...
const getParam = (
	...
	pipe(
		mergedTokenValues,
		MArray.unsafeGet(
			pipe(paramIndexToMergedTokenIndex, MArray.unsafeGet(paramIndex)),
		),
	);
		...
		basisFromMergedTokens: {
			decoder: (paramIndexToMergedTokenIndex) => (mergedTokenValues) =>
				Either.gen(function* () {
					...
					const nextFirstYearWeekStartMs = utils.utcTimeOfYearFirstWeekStart(
						y + 1,
					);
					const W = yield* pipe(
						1,
						getAndCheck({
							...
							min: 1,
							max: Math.floor(
								(nextFirstYearWeekStartMs - firstYearWeekStartMs) /
									utils.WEEK_MS,
							),
						}),
					);
	...
	Record.map((group, mergedBasis) => ({
		defaultValue: pipe(
			...
			Option.getOrThrowWith(
				() =>
					new Error(
						`Abnormal Error. ${mergedBasis} not found in MergedBasis.map`,
					),
			),
